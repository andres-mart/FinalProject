...
def preprocessing():
    df = pd.read_parquet(cf.DATASET_ROOT_PATH + "/yellow_tripdata_2022-05.parquet") ##change the path for your own local
    #df.sample(10, random_state=42) #seed of randomness for reproducibility

    df2 = pd.read_csv(cf.DATASET_ROOT_PATH + "/taxi+_zone_lookup.csv") 

    df3 = pd.read_csv(cf.DATASET_ROOT_PATH + "/taxi_zone_lookup_coordinates.csv")
